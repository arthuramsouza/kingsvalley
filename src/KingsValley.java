...
public class KingsValley extends UnicastRemoteObject implements IKingsValley {
  ...
  public int temPartida(int uid) throws RemoteException {
    ...
    if(p == null) {
      return -1;  /* Erro, jogador nao registrado */
    }
    ...
    /* Verifica se existe alguma partida aguardando player2 */
    matches_writeLock.lock();
    try {
      for(Match m : matches) {
        if(!m.isReady()) {
    ...
    /* Senao, cria uma nova partida aguardando player2 */
    matches_writeLock.lock();
    try {
      Match m = new Match(p);
      matches.add(m);
  ...
  /* Posiciona uma nova peca no tabuleiro */
  public int posicionaPeca(int uid, int posicao, int orientacao)
    ...
    return m.posicionaPeca(posicao, orientacao);
  }


  /* TODO implementar, testar */
  public int movePeca(int uid, int posicaoAtual, int dirDeslocamento,
                      int numCasas, int orientacaoNova) throws RemoteException {
    Player p = getPlayerById(uid);
    if(p == null) {
      return -1;  /* Parametros invalidos */
    }
    ...
    return m.movePeca(posicaoAtual, dirDeslocamento, numCasas, orientacaoNova);
  }
  ...
  public boolean hasPutAllPieces(int uid) throws RemoteException {
    Player p = getPlayerById(uid);
